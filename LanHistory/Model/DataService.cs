...
namespace Olbert.LanHistory.Model
{
    ...
    public class DataService : IDataService
    {
        ...
        public DateTime GetLastBackup()
        {
            ...
            try
            {
                using( RegistryKey reg = RegistryKey.OpenBaseKey( RegistryHive.CurrentUser, RegistryView.Default ) )
                {
                    using( RegistryKey key =
                        reg.OpenSubKey( @"Software\Microsoft\Windows\CurrentVersion\FileHistory" ) )
                    {
                        long lastBackup = (long?) key?.GetValue( "ProtectedUpToTime" ) ?? 0;

                        if( lastBackup <= 0 )
                            _logger.Error(
                                $"Could not find registry key {RegistryHive.CurrentUser.ToString()}\\Software\\Microsoft\\Windows\\CurrentVersion\\FileHistory" );
                        else
                        {
            ...
            {
                _logger.Error( e, "Exception thrown when getting last backup DateTime" );
            }
        ...
        public LanHistory GetLanHistory()
        {
            ...
            try
            {
                using( RegistryKey reg = RegistryKey.OpenBaseKey( RegistryHive.LocalMachine, RegistryView.Default ) )
                {
                        ...
                        reg.OpenSubKey( @"SYSTEM\CurrentControlSet\Services\fhsvc\Parameters\Configs" ) )
                    {
                        ...
                        {
                            _logger.Error(
                                $"Failed to parse File History configuration file, message was {e.Message}" );
                            return null;
                        }
                        ...
                        {
                            _logger.Error(
                                "Could not locate ServerName information in File History configuration file" );
                            return null;
                        }
                        else
                            _logger.LogDebugInformation(
                                "found ServerName element in File History configuration file" );
                        ...
                        {
                            _logger.LogDebugInformation(
                                "found TargetDriveType element in File History configuration file" );
                        ...
                        if( configValid )
                        {
                            ...
                            else
                            {
                                ...
                                catch( Exception e )
                                {
                                    configValid = false;
                                    _logger.Error(
                                        $"Failed to find MAC address for backup target, message was {e.Message}" );
                                }
                            }
                        }
                    }
            ...
            {
                _logger.Error( e, "Exception thrown while getting File History Backup configuration" );
            }
